...
pacman -S plasma-meta kde-graphics-meta kde-multimedia-meta kde-network-meta kde-sdk-meta kde-system-meta kde-utilities-meta ttf-dejavu breeze xdg-user-dirs plasma-login-manager pulseaudio pulseaudio-bluetooth firewalld ipset thunderbird firefox keepassxc bluez bluez-utils networkmanager-vpnc htop gimp vlc libreoffice-fresh gparted ntfs-3g git nm-connection-editor acpid dbus avahi cups nss-mdns qt5-wayland android-tools scrcpy system-config-printer linux-headers dkms dosfstools tree power-profiles-daemon xdg-desktop-portal efibootmgr plymouth refind rsync networkmanager-openconnect efitools sbsigntools sbctl tcpdump usbutils cdrtools libisoburn cdrdao libburn dvd+rw-tools ccd2iso sox libcdio audiocd-kio k3b libdvdread libdvdcss libdvdnav libaacs libbluray wget
...
# Install NVIDIA drivers and adding MODULES to initramfs
verbose "Installing NVIDIA drivers and adding MODULES to initramfs."
pacman -S nvidia-dkms
sed -i 's/^MODULES=.*/MODULES=(atlantic nvidia nvidia_modeset nvidia_uvm nvidia_drm vfat fat)/' /etc/mkinitcpio.conf
mkinitcpio -P
